...
tracker_list=$(curl -Ns https://raw.githubusercontent.com/zoeyzsz/trackerslist/master/trackers_all.txt | awk '$1' | tr '\n' ',')
...
aria2c --enable-rpc --rpc-listen-all=false --rpc-listen-port 6800 --check-certificate=false\
   ...
   --bt-tracker="[$tracker_list]" --bt-max-peers=0 --seed-time=0.01 --seed-ratio=1.0 --min-split-size=10M \
